...
- [ ] **EmDash CMS — Phase 3: automated provisioning (the platform)** (branch `feature/emdash-phase3`) — IN PROGRESS. Zero-click per-publication EmDash instance via Workers for Platforms + a provisioning Dynamic Workflow. Plan: `.agents/plans/emdash-fleet-implementation.md` (Phase 3). Decisions (2026-06-24): provisioner = new `services/provisioner`; managed = shared bundle, no core customization (Track B).
  ...
  - [ ] **Spike #1 (gated on scoped CF API token)** — prove the raw-API dispatch script+assets upload (`cf-api.uploadDispatchScript` + asset-manifest hash spec + `buildTenantBindings` type strings) from HTTP, since the Workflow Worker can't shell out to wrangler.
