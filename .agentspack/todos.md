...
- [x] **EmDash CMS Provider — Phase 1 (per-publication write path)** — Made the content write-path CMS-selectable per publication, with EmDash as a working second provider behind one test publication. SonicJS publications behave identically (regression-clean). See `.agents/plans/emdash-fleet-implementation.md` (Phase 1) and `docs/emdash-integration-guide.md`. Includes:
  - DAL migration (`0022_cms_provider.sql`): `cms_provider` (TEXT NOT NULL DEFAULT 'sonicjs'), `cms_base_url`, `cms_token` (AES-GCM encrypted via `TOKEN_ENCRYPTION_KEY`), `cms_provisioning_status`, `cms_instance_meta` on `publications`
  - Data layer: `CmsProvider`/`CmsProvisioningStatus`/`PublicationCmsCredentials` types, new columns on Publication/Create/Update inputs, `getPublicationWithCmsToken()` (decrypts token; never exposed by ordinary reads — mirrors social_connections). Extracted shared `crypto.ts` (AES-GCM) used by both social-connections and publications
  - `@hotmetal/shared`: promoted `CmsApi` → `CmsClient` interface + `SonicCmsClient` (byte-identical impl; `CmsApi` kept as deprecated alias). New `EmdashCmsClient` (markdown→Portable Text on write via `emdash/client`, PT→markdown on read, `ec_pat_` bearer auth, create-as-draft-then-publish, status mapped via `hm_status`, all Post/Rendition fields). New `getCmsClient(publication, dal, env)` factory + `EmdashCmsClientUnavailableError`
  - Migrated ~15 write-path call sites to the factory (apps/web: cms-publish tool, writer-agent, api/publications, api/sessions, agents-api/v1/publications; services/publisher: feed-store, publish routes + adapters typed to `CmsClient`). Publisher blog routes gained optional `publicationId`
  - EmDash collection seed (`emdash/seed.json` + README): `posts` + `renditions` collections carrying every field; documents EmDash reserved-system-column gotcha (`hm_*`, `rendition_status`, `outlet_published_at` slugs)
  - Integration test (`scripts/emdash-integration-test.ts`): validated end-to-end against a real EmDash 0.22.0 instance (the spike) — all 28 checks pass (create/publish/read lossless incl. citations + PT body, list filtering, update-to-publish, renditions). Regression: publisher 22/25 tests pass (3 failures are the pre-existing stale `/posts/` URL assertions, unrelated)
- [ ] **EmDash CMS — Phase 3: automated provisioning (the platform)** (branch `feature/emdash-phase3`) — IN PROGRESS. Zero-click per-publication EmDash instance via Workers for Platforms + a provisioning Dynamic Workflow. Plan: `.agents/plans/emdash-fleet-implementation.md` (Phase 3). Decisions (2026-06-24): provisioner = new `services/provisioner`; managed = shared bundle, no core customization (Track B).
  - [x] **Spike #0 (GO/NO-GO) — GREEN, proven live** on prod account (WfP purchased + enabled). #0a: emdash-blog bundle deployed to dispatch namespace `hotmetal-emdash`, 7.4 MB admin SPA served `200`, script 1.87 MB gzip ≪ 10 MB cap. #0b: auto-migrate on remote-D1 first boot + headless seed (admin + `ec_pat_` minted via `@emdash-cms/auth`, Worker-safe) → `POST /posts`→publish→read, zero clicks. See memory `emdash-phase3-spike0`.
  - [x] **Scaffold `services/provisioner`** — Worker + Hono entry (`/api/provision`, `/status`, `/teardown`), `ProvisionWorkflow` (create-d1→r2→kv→upload-script→trigger-migrate→bootstrap→store-credentials→wire-hostname→mark-ready; failure→mark-failed, infra left for idempotent retry), Worker-safe `bootstrap.ts` (4 passing tests), `cf-api.ts` (idempotent ensure* + D1 query + custom-hostname + teardown, 30s timeouts), `tenant.ts` (naming + binding metadata), `bundle.ts` + `scripts/release-bundle.ts` (R2 release model). Typecheck clean; 2 senior-code-reviewer passes clean (idempotent creates, no destructive auto-teardown, retry config, auth/teardown gating, concurrency dedup).
  - [x] **Spike #1 — GREEN** — proved the raw-API dispatch script+assets upload via the real `cf-api.uploadDispatchScript`: 374-module + 53-asset emdash-blog build uploaded as `spike1-tenant`, served the 7.4 MB admin SPA + content API with a reused PAT. Asset-manifest hash spec (leading 32 hex of SHA-256) + binding type strings (`worker_loader`/`images`/`d1`/`r2_bucket`/`kv_namespace`) confirmed live. Caveats removed from cf-api/tenant.
  - [x] **Dispatch routing by Host** — `apps/publications-web` middleware (`src/middleware.ts` + `lib/emdash-dispatch.ts`) forwards `emdash`+`ready` publications (resolved by slug/custom-domain) to their tenant script via a new `DISPATCHER` dispatch-namespace binding; legacy SonicJS renders unchanged; fail-loud 502 + log if a ready tenant is missing/broken. Removed the provisioner `wire-hostname` step (it wrongly set custom_domain=subdomain → redirect loop). Reviewed clean.
  - [x] **Trigger on publication-create** — `apps/web` create handler resolves `cmsProvider` (body or `DEFAULT_CMS_PROVIDER`, default 'sonicjs'); EmDash → create as `provisioning` + `triggerEmdashProvision` (PROVISIONER service binding, non-blocking, marks `failed` on trigger error). Status surfaced via existing `GET /publications/:id` (returns cms_provisioning_status). `DEFAULT_CMS_PROVIDER` stays 'sonicjs' until e2e validation.
  - [x] **End-to-end live validation** (joint, touches prod): provisioner deployed w/ secrets, bundle released, publication auto-provisioned + rendered, `DEFAULT_CMS_PROVIDER='emdash'` flipped live (super-emdash.hotmetalapp.com). See memory `emdash-phase3-spike0`.
  - [x] **Finish-line polish** (branch `feature/emdash-phase3`) — runbook `docs/emdash-phase3-runbook.md`. All reviewed clean:
    - [x] **Deprovision-on-publication-delete** — `teardownTenant` reports per-resource failures (no silent leak); `resolveTenantResources` derives names when `cms_instance_meta` is absent; `/api/teardown` rewritten; both web + agents-api delete handlers tear down the tenant before `deletePublication` (502 + no-delete on failure). Dead test tenants cleaned (spike + `pub-6007ed1f…`); live super-emdash + the separate April `emdash-site*` project preserved.
    - [x] **Comments on the fleet** — found + fixed: provisioned tenants were missing both Turnstile keys (submit 503 + UI hidden). Provisioner now injects `TURNSTILE_SITE_KEY` (var) + `TURNSTILE_SECRET_KEY` (new `secret_text` support), both-or-neither + warn. Needs provisioner secret set + super-emdash re-provision to take effect live.
    - [x] **Lock down `emdash-dispatch`** — `workers_dev:false` (service-binding-only).
    - [x] **Review nits** — regex→`Workflow.get()` precheck/recheck; shared `parseCmsInstanceMeta`; pub-web double-lookup → `locals.resolvedPublication` + `resolvePublicationForPage` (10 pages).
    - [ ] **Deploy + re-provision super-emdash (Turnstile) + live comment test; PR `feature/emdash-phase3` → main.**
